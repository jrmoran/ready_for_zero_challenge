...
# Now, what I want is to take the first line of the file 
# txt = IO.readlines('readyforzero1.txt')[0].chomp

# Storing all sequences of five characters into an array
cqns = txt.scan /.{5}/

matches = []
(79..cqns.length).each do |i| 
  print " #{i} |"
  (i..cqns.length - 1).each do |j| 
    matches << { :a => cqns[i], :b => cqns[j] } if levenshtein(cqns[i], cqns[j]) == 1
  end
...
# puts cqns.join ','
puts "\n==== MATCHES ===="
puts matches.join ','       # {:a=>'i2d+r', :b=>'i2Z+r'}

# This line takes all continuous sequences of 5 characters
# ( 0..txt.length - 5).each { |i| sequences << txt[ i .. i + 4 ] }
